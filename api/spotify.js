import express from "express";
import dotenv from "dotenv";
import fetch from "node-fetch";
import { Buffer } from "buffer";
import ColorThief from "colorthief";
import path from "path";
import fs from "fs";

dotenv.config();
const app = express();

const PLACEHOLDER_IMAGE =
  "";
const PLACEHOLDER_URL = "https://source.unsplash.com/random/300x300/?aerial";
const SPOTIFY_CLIENT_ID = process.env.SPOTIFY_CLIENT_ID;
const SPOTIFY_SECRET_ID = process.env.SPOTIFY_SECRET_ID;
const SPOTIFY_REFRESH_TOKEN = process.env.SPOTIFY_REFRESH_TOKEN;

let SPOTIFY_TOKEN = "";

const REFRESH_TOKEN_URL = "https://accounts.spotify.com/api/token";
const NOW_PLAYING_URL = "https://api.spotify.com/v1/me/player/currently-playing";
const RECENTLY_PLAYING_URL =
  "https://api.spotify.com/v1/me/player/recently-played?limit=10";

// ---------------- Helper Functions ---------------- //

const getAuth = () => {
  return Buffer.from(`${SPOTIFY_CLIENT_ID}:${SPOTIFY_SECRET_ID}`).toString("base64");
};

const refreshToken = async () => {
  const response = await fetch(REFRESH_TOKEN_URL, {
    method: "POST",
    headers: {
      Authorization: `Basic ${getAuth()}`,
      "Content-Type": "application/x-www-form-urlencoded",
    },
    body: new URLSearchParams({
      grant_type: "refresh_token",
      refresh_token: SPOTIFY_REFRESH_TOKEN,
    }),
  });
  const data = await response.json();
  if (!data.access_token) throw new Error("Failed to refresh token");
  return data.access_token;
};

const get = async (url) => {
  if (!SPOTIFY_TOKEN) SPOTIFY_TOKEN = await refreshToken();

  let response = await fetch(url, {
    headers: { Authorization: `Bearer ${SPOTIFY_TOKEN}` },
  });

  if (response.status === 401) {
    SPOTIFY_TOKEN = await refreshToken();
    response = await fetch(url, { headers: { Authorization: `Bearer ${SPOTIFY_TOKEN}` } });
  }

  if (response.status === 204) throw new Error(`${url} returned no data.`);

  return response.json();
};

const barGen = (barCount) => {
  let barCSS = "";
  let left = 1;
  for (let i = 1; i <= barCount; i++) {
    let anim = Math.floor(Math.random() * 500) + 500;
    let x1 = Math.random();
    let y1 = Math.random() * 2;
    let x2 = Math.random();
    let y2 = Math.random() * 2;

    barCSS += `.bar:nth-child(${i}) { left: ${left}px; animation-duration: 15s, ${anim}ms; animation-timing-function: ease, cubic-bezier(${x1},${y1},${x2},${y2}); }`;
    left += 4;
  }
  return barCSS;
};

const gradientGen = async (imageURL, colorCount) => {
  const imageBuffer = await fetch(imageURL).then((res) => res.arrayBuffer());
  const palette = await ColorThief.getPalette(Buffer.from(imageBuffer), colorCount);
  return palette;
};

const loadTemplate = () => {
  try {
    const file = fs.readFileSync(path.join("api", "templates.json"), "utf8");
    const templates = JSON.parse(file);
    return templates.templates[templates["current-theme"]];
  } catch (err) {
    console.error("Failed to load template:", err);
    return "spotify.html.j2";
  }
};

const loadImageB64 = async (url) => {
  const res = await fetch(url);
  const buffer = await res.arrayBuffer();
  return Buffer.from(buffer).toString("base64");
};

// ---------------- Main Function to Generate SVG ---------------- //
const makeSVG = async (data, background_color, border_color) => {
  const barCount = 84;
  const contentBar = Array(barCount).fill("<div class='bar'></div>").join("");
  const barCSS = barGen(barCount);

  let item;
  let currentStatus;

  if (!("is_playing" in data)) {
    currentStatus = "Recently played:";
    const recentPlays = await get(RECENTLY_PLAYING_URL);
    const itemIndex = Math.floor(Math.random() * recentPlays.items.length);
    item = recentPlays.items[itemIndex].track;
  } else {
    item = data.item;
    currentStatus = "Vibing to:";
  }

  let image, barPalette, songPalette;

  if (!item.album.images || item.album.images.length === 0) {
    image = PLACEHOLDER_IMAGE;
    barPalette = await gradientGen(PLACEHOLDER_URL, 4);
    songPalette = await gradientGen(PLACEHOLDER_URL, 2);
  } else {
    image = await loadImageB64(item.album.images[1].url);
    barPalette = await gradientGen(item.album.images[1].url, 4);
    songPalette = await gradientGen(item.album.images[1].url, 2);
  }

  const dataDict = {
    contentBar,
    barCSS,
    artistName: item.artists[0].name.replace(/&/g, "&amp;"),
    songName: item.name.replace(/&/g, "&amp;"),
    songURI: item.external_urls.spotify,
    artistURI: item.artists[0].external_urls.spotify,
    image,
    status: currentStatus,
    background_color,
    border_color,
    barPalette,
    songPalette,
  };

  const template = loadTemplate();
  // Here you would use a template engine like EJS, Handlebars, or similar
  return JSON.stringify(dataDict); // placeholder for actual HTML render
};

// ---------------- Routes ---------------- //
app.get(["/", "/:path", "/with_parameters"], async (req, res) => {
  const background_color = req.query.background_color || "181414";
  const border_color = req.query.border_color || "181414";

  let data;
  try {
    data = await get(NOW_PLAYING_URL);
  } catch (error) {
    data = await get(RECENTLY_PLAYING_URL);
  }

  const svg = await makeSVG(data, background_color, border_color);
  res.setHeader("Content-Type", "image/svg+xml");
  res.setHeader("Cache-Control", "s-maxage=1");
  res.send(svg);
});

// ---------------- Start Server ---------------- //
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`Server running on port ${PORT}`));
